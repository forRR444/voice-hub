...
// 彩度を抑えた色を生成（背景用）
function desaturatedTint(rgb: [number, number, number], lightness: number, saturationRatio: number): string {
...
export function generateThemeFromAccent(accent: string): SalonThemeConfig {
  ...
  return {
    ...
    cardBg: "#FFFFFF",
    cardBorder: isDark ? "#E0E0E0" : desaturatedTint(rgb, 0.90, satRatio),
