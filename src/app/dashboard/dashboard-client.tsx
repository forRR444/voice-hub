    ...
    <div className="max-w-5xl mx-auto">
        ...
        <div className="relative">
          ...
          {showAddMenu && (
            <>
              ...
              <div
                className="absolute right-0 top-12 z-50 w-60 rounded-[4px] py-1.5"
                style={{ background: white, boxShadow: floatShadow }}
              >
                <button
                  onClick={() => {
      ...
      {stats.total > 0 && (
        <>
          ...
          <div
            className="sm:hidden rounded-[4px] flex mb-6 animate-fade-in-delay-1"
          ...
          >
            {[
              ...
              <div key={s.label} className="flex-1 py-3.5 text-center">
                <span
                  className="block text-[11px] font-medium uppercase mb-1"
                  style={{ color: slate, letterSpacing: "0.04em" }}
                >
                  {s.label}
                </span>
                <span
            ...
            {[
              {
                label: "合計",
                value: stats.total,
                icon: <MessageSquare size={14} style={{ color: slate }} />,
              },
              {
                label: "承認済み",
                value: stats.approved,
                icon: <CheckCircle size={14} style={{ color: slate }} />,
              },
              {
                label: "未承認",
                value: stats.pending,
                icon: <CircleDot size={14} style={{ color: slate }} />,
              },
              {
                label: "平均評価",
                value: stats.avg > 0 ? stats.avg.toFixed(1) : "-",
                icon: <Star size={14} style={{ color: slate }} />,
              },
            ].map((s) => (
      ...
      {stats.total > 0 && (
        <div
          className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-0 animate-fade-in-delay-2"
          ...
          <div className="flex gap-0 overflow-x-auto">
            {tabs.map((tab) => (
              <button
                ...
                style={{
                  color: filter === tab.key ? ink : slate,
...
function GuideCards() {
  ...
  useEffect(() => {
    /* eslint-disable react-hooks/set-state-in-effect -- hydration-safe localStorage read on mount */
    try {
      setHidden(JSON.parse(localStorage.getItem("voicehub_guide_hidden") || "{}"));
    } catch {
      /* noop */
    }
    setMounted(true);
  ...
  const items = [
    {
      key: "welcome",
      text: "ご登録ありがとうございます！まずはフォームを設定して、お客様にLINEやメールでURLを送りましょう。回答がここに届きます。",
      link: "/dashboard/forms",
      cta: "フォームを設定する →",
    },
    {
      key: "widget",
      text: "お客様の声をホームページに表示して、信頼度をアップさせましょう。ウィジェットから簡単に埋め込めます。",
      link: "/dashboard/widgets",
      cta: "ウィジェット設定へ →",
    },
    {
      key: "sns",
      text: "口コミからInstagramやSNS用の画像を自動生成できます。お客様の声を投稿してフォロワーに信頼を伝えましょう。",
      link: "/dashboard/sns",
      cta: "SNS画像を作成する →",
    },
  ];
          ...
          <div key={g.key} className="py-5" style={{ borderBottom: `1px solid ${rule}` }}>
            <div className="flex items-start justify-between gap-4">
                ...
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span
                      ...
                      style={{ color: brand, background: `${brand}10`, letterSpacing: "0.04em" }}
                    >
                      ご案内
                    </span>
                  </div>
                  <p className="text-sm leading-[1.6]" style={{ color: slate }}>
                    {g.text}
                  </p>
                  <Link
                    href={g.link}
                    className="inline-block mt-2 text-sm font-medium transition-opacity duration-150 hover:opacity-70"
                    style={{ color: brandD }}
                  >
                    {g.cta}
                  </Link>
              ...
              <button
                ...
                className="transition-opacity duration-150 hover:opacity-50 cursor-pointer mt-1"
                style={{ color: muted }}
              >
                <XCircle size={16} />
              </button>
  ...
  return (
    <div className="group py-5" style={{ borderBottom: isLast ? "none" : `1px solid ${rule}` }}>
      <div className="flex items-start gap-3.5">
        <div
          className="w-10 h-10 rounded-full flex items-center justify-center shrink-0 text-xs font-bold text-white mt-0.5"
          style={{ background: `linear-gradient(135deg, ${brand} 0%, ${brandD} 100%)` }}
        >
          {initials}
        </div>
        <div className="flex-1 min-w-0">
            ...
            <div>
              <div className="flex items-center gap-2">
                <Link
                  ...
                  style={{ color: brand, letterSpacing: "-0.011em" }}
                >
                  {t.name || "お客様"}
                </Link>
                {(t.title || t.company) && (
          ...
          <p
            className="text-sm leading-[1.6] mt-1.5 line-clamp-3"
            ...
            <div className="flex gap-1.5 mt-2.5">
              {t.tags.map((tag) => (
                <span
                  ...
                  style={{ background: canvas, color: slate }}
                >
                  {tag}
                </span>
              ))}
...
function StatusPill({
  ...
  status: string;
  onSelect: (s: "approved" | "rejected" | "pending") => void;
}) {
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      >
        ...
        {status === "pending" && (
          <span className="w-[6px] h-[6px] rounded-full" style={{ background: muted }} />
        )}
        <ChevronDown size={12} />
      </button>
      {open && (
        <div
          className="absolute right-0 top-9 z-30 w-40 sm:w-48 rounded-lg py-1.5 shadow-lg"
